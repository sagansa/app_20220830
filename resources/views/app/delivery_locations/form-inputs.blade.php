...
<div class="mt-6 space-y-6 sm:mt-5 sm:space-y-5">
    <x-input.text
        name="name"
        label="Name"
        value="{{ old('name', ($editing ? $deliveryLocation->name : '')) }}"
        maxlength="255"
        required
    ></x-input.text>
    ...
    <x-input.text
        name="contact_name"
        label="Contact Name"
        value="{{ old('contact_name', ($editing ? $deliveryLocation->contact_name : '')) }}"
        maxlength="255"
        required
    ></x-input.text>
    ...
    <x-input.text
        ...
        value="{{ old('contact_number', ($editing ? $deliveryLocation->contact_number : '')) }}"
        maxlength="255"
        required
    ></x-input.text>

    <x-input.textarea name="Address" label="Address" maxlength="255" required
        >{{ old('Address', ($editing ? $deliveryLocation->Address : ''))
        }}</x-input.textarea
    >
    ...
    <x-input.textarea name="notes" label="Notes" maxlength="255"
        >{{ old('notes', ($editing ? $deliveryLocation->notes : ''))
        }}</x-input.textarea
    >
    ...
    <x-shows.dl>
        ...
        <x-shows.sub-dl>
            ...
            <x-shows.dd
                >{{ optional($deliveryLocation->created_by)->name ?? '-' }}
            </x-shows.dd>
        </x-shows.sub-dl>
        ...
        <x-shows.sub-dl>
            ...
            <x-shows.dd
                >{{ optional($deliveryLocation->approved_by)->name ?? '-' }}
            </x-shows.dd>
        </x-shows.sub-dl>
        @endrole
    </x-shows.dl>
