...
<div class="mt-6 space-y-6 sm:mt-5 sm:space-y-5">
    <x-input.image name="image" label="Image">
        <div x-data="imageViewer('{{ $editing && $invoicePurchase->image ? \Storage::url($invoicePurchase->image) : '' }}')" class="mt-1 sm:mt-0 sm:col-span-2">
            <!-- Show the image -->
    ...
    <x-input.date name="date" label="Date"
        value="{{ old('date', $editing ? optional($invoicePurchase->date)->format('Y-m-d') : '') }}" max="255"
        required></x-input.date>

    <x-inputs.hidden name="discounts" :value="old('discounts', $editing ? $invoicePurchase->discounts : '0')"></x-inputs.hidden>
        ...
        <x-shows.dl>
            ...
            @role('super-admin|manager|supervisor')
                <x-shows.sub-dl>
                    ...
                    <x-shows.dd>{{ optional($invoicePurchase->created_by)->name ?? '-' }}
                    </x-shows.dd>
                </x-shows.sub-dl>
            @endrole
        </x-shows.dl>
