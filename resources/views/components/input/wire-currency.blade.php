<td>
    <form wire:submit.prevent="{{ $wiresubmit }}" autocomplete="off">
        <div
            ...
            <input type="number" wire:model.defer="{{ $wiremodel }}"
                {{ $attributes->merge([
                    'class' =>
                        'block w-full p-0 text-xs font-medium text-right text-gray-500 placeholder-gray-500 border-0 focus:ring-0',
                ]) }}>
            @error($name)
