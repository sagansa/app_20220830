...
<x-input.partials.group-head>
    ...
    <x-input.partials.group>
        <x-input.basic-currency class="text-right" type="number" :name="$name" label="{{ $label ?? '' }}"
            :value="$value ?? ''" :attributes="$attributes" :min="$min" :max="$max" :step="$step">
        </x-input.basic-currency>
    </x-input.partials.group>
</x-input.partials.group-head>
